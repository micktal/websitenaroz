          ...
          sessions.map((s, idx) => {
            ...
            return (
              <Card key={idx} className="relative overflow-hidden hover:shadow-lg transition-all group">
                ...
                {/* Main clickable area */}
                <LinkWrapper to={to} className="block p-6 hover:bg-gray-50 transition-colors">
                  {/* Date range */}
                  <div className="flex items-center gap-2 text-xs text-muted-foreground mb-3">
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
                    <span>{s['Start Date']} — {s['End Date']}</span>
                  </div>
      ...
      {sessions.length > 0 && (
        <Card className="mt-12 p-6 bg-gray-50 text-center">
          <p className="text-sm text-muted-foreground">
            Can't find a suitable date? <LinkWrapper to="/contact" className="text-primary hover:underline font-medium">Contact us</LinkWrapper> to discuss custom scheduling or enquire about future sessions.
          </p>
        </Card>
      )}
    </div>
